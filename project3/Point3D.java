...
public class Point3D implements Comparable<Point3D> {
    ...
    public double distance(Point3D that) {
       return Math.sqrt((that.x - x) * (that.x - x)
              + (that.y - y)* (that.y - y) + (that.z - z) * (that.z - z));
    }
    ...
    Point3D o = new Point3D(0, 0, 0);
        if (distance(o) < that.distance(o)) return -1;
        else if (distance(o) == that.distance(o)) return 0;
        else return 1;
    }
